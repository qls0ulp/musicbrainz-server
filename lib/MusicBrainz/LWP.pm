...
use DBDefs;
use HTTP::Status qw( HTTP_SERVICE_UNAVAILABLE );
use LWP::UserAgent;
...
sub get { shift->_timely_call_method('get', 'GET', @_); }
sub mirror { shift->_timely_call_method('mirror', 'GET', @_); }
sub post { shift->_timely_call_method('post', 'POST', @_); }
sub request {
    my ($self, $request) = @_;
    $self->_timely_call_method('request', $request->method, $request);
}
